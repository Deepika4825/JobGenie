        ...
        <div className="px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              <p className="text-sm font-semibold truncate" style={{ color: '#4A4A4A' }}>{profile.name || 'User'}</p>
            ...
            <NavLink key={to} to={to} onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                  isActive ? 'text-white shadow-sm' : 'text-gray-600'
                }`
              }
              style={({ isActive }) => isActive ? { background: '#8E4585' } : {}}
              onMouseEnter={e => { if (!e.currentTarget.classList.contains('text-white')) e.currentTarget.style.background = '#f5eef5'; }}
