...
DB_PATH      = os.path.join(os.path.dirname(__file__), "jobgenie.db")
print("Backend ready - SQLite:", DB_PATH)
...
def signup():
    ...
    try:
        db.execute(
            ...
            (name, username, email, hash_pw(password))
        )
        db.commit()
...
# â”€â”€ Profile routes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def update_profile(user_id):
    ...
    db   = get_db()
    db.execute("""
        INSERT INTO profiles (user_id, phone, location, bio, degree, college,
            graduation_year, job_title, experience, interests, resume_name, avatar, updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))
        ON CONFLICT(user_id) DO UPDATE SET
            phone=excluded.phone, location=excluded.location, bio=excluded.bio,
            degree=excluded.degree, college=excluded.college,
            graduation_year=excluded.graduation_year, job_title=excluded.job_title,
            experience=excluded.experience, interests=excluded.interests,
            resume_name=excluded.resume_name, avatar=excluded.avatar,
            updated_at=excluded.updated_at
    """, (
        user_id,
        data.get("phone",""), data.get("location",""), data.get("bio",""),
        data.get("degree",""), data.get("college",""), data.get("graduationYear",""),
        data.get("jobTitle",""), data.get("experience",""),
        json.dumps(data.get("interests",[])),
        data.get("resumeName",""), data.get("avatar","")
    ))
    db.commit()
    return jsonify({"success": True}), 200


# â”€â”€ Resume analysis â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def fetch_jobs(job_titles, location="India", experience=""):
    ...
    for title in job_titles[:4]:
        try:
            resp = requests.get(JSEARCH_URL, headers=headers,
                params={"query": f"{title} {loc_q}", "num_pages": "1",
                        "page": "1", "date_posted": "month", "country": "in"}, timeout=10)
            ...
            for j in resp.json().get("data", []):
                ...
                if ps:
                    try:
                        ...
                        if (datetime.now(timezone.utc) - dt).days > 30: continue
                    except: pass
                ...
                all_jobs.append({
                    ...
                    "posted":          format_posted(ps)
                })
                if len(all_jobs) >= 2: break
        except Exception as e:
            print(f"JSearch error [{title}]:", e)
    return all_jobs if all_jobs else fallback_jobs(job_titles, location)


def fallback_jobs(job_titles, location="India"):
    portals    = [("LinkedIn","https://www.linkedin.com/jobs/search/?keywords={}"),
    ...
    locs       = ["Bangalore, Karnataka, India","Mumbai, Maharashtra, India",
                  "Hyderabad, Telangana, India","Chennai, Tamil Nadu, India","Pune, Maharashtra, India"]
    jobs = []
    for i, title in enumerate(job_titles):
        slug = title.lower().replace(" ","-")
        src, link = portals[i % len(portals)]
        jobs.append({"role": title, "company": "Multiple Companies",
                     "location": locs[i % len(locs)], "work_type": work_types[i % len(work_types)],
                     "employment_type": "Full-time",
                     "description": f"Explore {title} opportunities.",
                     "apply_link": link.format(slug), "source": src, "posted": "Today"})
    return jobs
...
def upload():
    ...
    try:
        ...
        if user_id:
            db = get_db()
            db.execute("""INSERT INTO resume_analyses
                (user_id, filename, score, skills, skill_gaps, suggestions)
                VALUES (?,?,?,?,?,?)""",
                (int(user_id), file.filename,
